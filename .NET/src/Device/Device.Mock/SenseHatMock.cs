...
{
    public class SenseHatMock: ITemperatureSensor, IPressureSensor, IHumiditySensor, ILedMatrix
    {
        ...
        public void DimMatrix(TimeSpan period, Color originalColor)
        {
            FillMatrix(Color.Black);
            Thread.Sleep(period);
            FillMatrix(originalColor);
        }
    }
}