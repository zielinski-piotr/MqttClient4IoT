...
public class MqttClient : IMqttClient, IDisposable
{
    ...
    private void SetupReconnectOnDisconnected(bool autoReconnect,
        Func<CancellationToken, Task>? onDisconnectedHandlerAsync)
    {
        ...
        _client.DisconnectedAsync += async args =>
        {
            ...
            if (onDisconnectedHandlerAsync is not null)
            {
                try
                {
                    ...
                    _logger.LogInformation("Invoked OnDisconnectedHandlerAsync user handler");
                }
                catch (Exception e)
                {
                    _logger.LogError(e,
                        "OnDisconnectedHandlerAsync user handler reported an exception. Exception was suppressed");
                }
            }
    ...
    private void SetupSubscriptionsOnConnected(IEnumerable<string> topicsToSubscribe,
        Func<CancellationToken, Task>? onConnectedHandlerAsync)
    {
        var topics = topicsToSubscribe as string[] ?? topicsToSubscribe.ToArray();
        ...
        _client.ConnectedAsync += async _ =>
        {
            if (onConnectedHandlerAsync is not null)
            {
                try
                {
                    ...
                    _logger.LogInformation("Invoked OnConnectedHandlerAsync user handler");
                }
                catch (Exception e)
                {
                    _logger.LogError(e,
                        "OnConnectedHandlerAsync user handler reported an exception. Exception was suppressed");
                }
            }

            if (topics.Any())
            {
                _logger.LogInformation("Trying to subscribe to topics: {Shared.Mqtt.Topics}",
                    string.Join(", ", topics.Select(x => $"'{x}'")));
                ...
                if (unsuccessfulResultItems.Any())
                {
                    _logger.LogError("Subscribing to topics failed:{Reasons}",
                        string.Join(Environment.NewLine,
                            unsuccessfulResultItems.Select(x =>
                                $"Topic: '{x.TopicFilter.Topic}' Reason: '{x.ResultCode}'")));
                    return;
                }

                _logger.LogInformation("Successfully subscribed to topics: {Shared.Mqtt.Topics}",
    ...
    private async Task ConnectAsync()
    {
        ...
        {
            _logger.LogError("Client connection error result: '{Result}'",
