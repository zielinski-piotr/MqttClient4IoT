...
using Mqtt.Device.MessageHandlers;
using Mqtt.Device.Services;
...
var builder = new HostBuilder()
    ...
    {
        services.AddOptions();
        services.AddSingleton<IHostedService, DeviceService>();

        services.RegisterMqttService();

        if (Runtime.RuntimeIdentifier is "linux-arm" or "linux-arm64" or "raspbian.11-arm")
        {
            services.RegisterRpiDeviceTypes();
        }
        else
        {
            services.RegisterMockDeviceTypes();
        }
